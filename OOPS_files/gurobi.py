...
def getMinSizeRealizableNoGurobi(pot, ratio):
    half_edges, half_edges_hat = get_half_edge_labels()
    matrix = []
    for tile in pot:
        sums = [0 for i in range(len(half_edges))]
        for item in tile:
            if item in half_edges:
                sums[half_edges.index(item)] = sums[half_edges.index(item)] + 1
            elif item in half_edges_hat:
                sums[half_edges_hat.index(item)] = sums[half_edges_hat.index(item)] - 1
        matrix.append(sums)
    matrix = np.transpose(matrix)
    ...
    # Actually build the orientation
    num_nodes = sum(ratio)

    rem_slots = {}
    for heh in half_edges_hat:
        rem_slots.update({heh : []})

        for node_index in range(num_nodes):
            for tile in pot:
                if node_index in tile_assignments[tile]:
                    rem_slots.get(heh).append(tile.count(heh))
    
    # print(rem_slots)

    orientations = [[[0 for i in range(num_nodes)] for j in range(num_nodes)] for k in range(bond_edge_types)]

    for start_node in range(num_nodes):
        for tile in pot:
            if start_node in tile_assignments[tile]:
                for he in half_edges:
                    numhe = tile.count(he)
                    while(numhe > 0):
                        # print(numhe)
                        for end_node in range(num_nodes):
                            if(rem_slots[hat(he)][end_node] >= 1):
                                # print('a')
                                rem_slots[hat(he)][end_node] = rem_slots[hat(he)][end_node] - 1
                                numhe = numhe - 1
                                orientations[half_edges.index(he)][start_node][end_node] = orientations[half_edges.index(he)][start_node][end_node] + 1
                                break
    ...
    # End orientation building
    return (G, pot, tile_assignments, orientations)

def getMinSizeRealizable(pot, ratio):
    print("#####")
    print(pot)
    print(ratio)
    possible_half_edges, possible_half_edges_hat = get_half_edge_labels()
