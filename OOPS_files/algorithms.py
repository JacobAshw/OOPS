...
def loop_through_qvals_canonical(Graph: nx.graph, tile_types: int, bond_edge_types: int, gurobi_print: bool):
    #Set our initial qvalue limits
    minqs = [0 for i in range(tile_types)]
    q_under_equal = -1
    ...
    while(True):
        #get our pot
        pot, tile_assignments, orientations, qvals, savedata = optimal_pot_s2_canonical(Graph, bond_edge_types, tile_types, minqs, q_under_equal, gurobi_print)
...
def S2_tiles_canonical(Graph, gurobi_flag):
    #Solve S1
    pot, tile_assignments, orientation = S1_optimal_pot(Graph, gurobi_flag)
    ...
    while not solved:
        print("Checking B_2=" + str(bond_edge_types) + ", T_2=" + str(tile_types))
        solved, pot, tile_assignments, orientations = loop_through_qvals_canonical(Graph, tile_types, bond_edge_types, gurobi_flag)
        if not solved:
            bond_edge_types = bond_edge_types + 1
            if(bond_edge_types >= tile_types or bond_edge_types > bond_edge_type_maximum):
                bond_edge_types = bond_edge_type_minimum
...
def S2_bonds_canonical(Graph, gurobi_flag, tile_types, bond_edge_types):
    tile_types_maximum = Graph.number_of_nodes()
    bond_edge_type_minimum = 1
    for bond_edges in range(int(bond_edge_type_minimum), int(bond_edge_types)):
        for tiles in range(tile_types+1, tile_types_maximum + 1):
            print("Checking B_2=" + str(bond_edges) + ", T_2=" + str(tiles))
            solved, pot, tile_assignments, orientations = loop_through_qvals_canonical(Graph, tiles, bond_edges, gurobi_flag)
            if(solved):
                return pot, tile_assignments, orientations
    return False, False, False

def loop_through_qvals_canonical_hybrid(Graph: nx.graph, tile_types: int, bond_edge_types: int, gurobi_print: bool, partitions):
    #Set our initial qvalue limits
    minqs = [0 for i in range(tile_types)]
    q_under_equal = -1
    ...
    while(True):
        #get our pot
        pot, tile_assignments, orientations, qvals, savedata = optimal_pot_s2_canonical(Graph, bond_edge_types, tile_types, minqs, q_under_equal, gurobi_print, partitions)
...
def S2_tiles_hybrid_canonical(Graph, gurobi_flag):
    #Solve S1
    pot, tile_assignments, orientation = S1_optimal_pot(Graph, gurobi_flag)
    ...
    while not solved:
        print("Checking B_2=" + str(bond_edge_types) + ", T_2=" + str(tile_types))
        partitions = get_glist(num_to_partition, tile_types)
        solved, pot, tile_assignments, orientations = loop_through_qvals_canonical_hybrid(Graph, tile_types, bond_edge_types, gurobi_flag, partitions)
        if not solved:
            bond_edge_types = bond_edge_types + 1
            if(bond_edge_types >= tile_types or bond_edge_types > bond_edge_type_maximum):
                bond_edge_types = bond_edge_type_minimum
...
def S2_bonds_hybrid_canonical(Graph, gurobi_flag, tile_types, bond_edge_types):
    tile_types_maximum = Graph.number_of_nodes()
    bond_edge_type_minimum = 1
    for bond_edges in range(int(bond_edge_type_minimum), int(bond_edge_types)):
        for tiles in range(tile_types+1, tile_types_maximum + 1):
            print("Checking B_2=" + str(bond_edges) + ", T_2=" + str(tiles))
            partitions = get_glist(num_to_partition, tiles)
            solved, pot, tile_assignments, orientations = loop_through_qvals_canonical_hybrid(Graph, tiles, bond_edges, gurobi_flag, partitions)
            if(solved):
                return pot, tile_assignments, orientations
    return False, False, False